...
from core.strategy_engine import V18_CFG, DEFAULT_CFG
...
def run_sprint_backtest(capital: float, target: float, start: date, end: date) -> None:
    ...
    for label, old_v, new_v, unit in metrics_cmp:
        ...
        if unit == "EUR":
            ...
            dd_color = RED if label == "Max drawdown" else ""
    ...
    if doel_bereikt:
        ...
        print(f"  Tip: Verhoog risk_a of verleng periode.")
    ...
    for st in SIG_TYPES:
        ...
        base_risk = V18_CFG.get(
            "risk_a" if st == "A_EMACROSS"
            else "risk_b" if st in ("B_MACDCROSS", "E_BOS", "C_MOMENTUM")
            else "risk_c"
        )
    ...
    print(f"  Winstdoel     : {f'{GREEN}{BOLD}BEREIKT in {days_to_target} dagen{RESET}' if doel_bereikt else f'{RED}NIET bereikt{RESET}'}")
    ...
    print(f"  In core/strategy_engine.py is V18_CFG klaar.")
    print(f"  Om de live bot te updaten naar V18:")
    print(f"    from core.strategy_engine import V18_CFG")
    print(f"    # Gebruik V18_CFG in plaats van DEFAULT_CFG")
    print()
    ...
    print(f"  Trades opgeslagen: results/backtests/trades_v18_sprint_30k.csv")
