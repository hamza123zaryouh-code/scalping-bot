...
def run_backtest(capital: float, start: date, end: date) -> None:
    ...
    daily = trades_df.groupby("date")["pnl"].agg(
        dag_pnl="sum", trades="count", wins=lambda s: (s > 0).sum()
    ).reset_index()
    ...
    print(f"  {'Slechtste dag':<28} {RED}{worst_day['date']}{RESET}  "
          f"verlies: {RED}{worst_day['dag_pnl']:+,.2f} EUR{RESET}  "
          f"({worst_day['trades']:.0f} trades)")
    print(f"  {'Beste dag':<28} {GREEN}{best_day['date']}{RESET}  "
          f"winst: {GREEN}{best_day['dag_pnl']:+,.2f} EUR{RESET}  "
          f"({best_day['trades']:.0f} trades)")
    print(f"  {'Gem. verliesdag':<28} {clr(float(verlies_dgn['dag_pnl'].mean()) if not verlies_dgn.empty else 0, '+,.2f')} EUR")
    print(f"  {'Gem. winstdag':<28} {clr(float(winst_dgn['dag_pnl'].mean()) if not winst_dgn.empty else 0, '+,.2f')} EUR")
    ...
    for _, row in daily.nsmallest(5, "dag_pnl").iterrows():
        ...
        print(f"  {str(row['date']):<14} {clr(row['dag_pnl'], '+,.2f'):>23} "
    ...
    for month, grp in trades_df.groupby("month"):
        ...
        running += mpnl
        print(f"  {str(month):<10} {n:>8} {clr(mpnl, '+,.2f'):>23} "
    ...
    print(f"  {'Week':<10} {'Start (EUR)':>14} {'Eind (EUR)':>14} "
          f"{'Trades':>7} {'P&L':>12} {'Ret%':>7} {'Win%':>7}")
    print(f"  {'-' * 10} {'-' * 14} {'-' * 14} {'-' * 7} {'-' * 12} {'-' * 7} {'-' * 7}")

    if hasattr(svc, '_build_weekly_summary'):
        from backend.api.schemas.backtest import WeeklySummary
    ...
    print(f"  Max dagverlies      : {RED}{float(daily['dag_pnl'].min()):+,.2f} EUR{RESET}  "
          f"({str(worst_day['date'])})")
...
def _save_reports(trades_df, daily, sig_month, start, end) -> None:
    ...
    print(f"  Rapporten opgeslagen in results/backtests/")
