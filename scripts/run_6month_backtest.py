...
def run_6month_backtest(capital: float, start: date, end: date, version: str = "V20") -> None:
    ...
    print(f"  Max totaal DD   : {BOLD}{RED}{FTMO_MAX_TOT_PCT*100:.0f}% = EUR {capital*FTMO_MAX_TOT_PCT:,.0f}{RESET}")
    ...
    print(f"  {version} Config: risk_a={cfg['risk_a']*100:.1f}% | risk_b={cfg['risk_b']*100:.1f}% | risk_c={cfg['risk_c']*100:.1f}%")
    ...
    print(f"  Weekly compound: {'AAN' if cfg.get('weekly_compound') else 'UIT'} (+{cfg.get('compound_boost',1.1)*100-100:.0f}%/wk)")
    print()
    ...
    checks = [
        ...
        ("Piek-naar-trog DD %",
         ...
         f"info",
         True),
        ("Max lot size gebruikt",
         f"{max_lot_used:.2f} lots",
         f"{FTMO_MAX_LOT:.1f} lots",
         lot_cap_ok),
        ("Slechtste dag (verlies)",
         f"EUR {abs(worst_day_loss):,.0f}",
         f"EUR {FTMO_MAX_DAG_EUR:,.0f}",
         daily_dd_ok),
    ]
    ...
    running = capital
    print(f"  {'Maand':<10} {'Start':>12} {'Trades':>7} {'Win%':>6} {'P&L':>12} {'Ret%':>7} {'Max Lot':>9} {'Saldo':>12} {'Doel':>8}")
    ...
    if reset_weeks_cfg > 0:
        ...
        if n_resets > 0:
            ...
            print(f"  Totaal gecumuleerd    : EUR {total_banked:+,.0f}  "
                  f"({total_banked/capital*100:+.1f}%)")
        print(f"  Maanddoel stop        : EUR {monthly_target:,.0f}/maand")
        print(f"  Verwachte jaaropbrengst: EUR {total_pnl/6*12:+,.0f}  "
    ...
    for bin_label, row in lot_dist.iterrows():
        ...
        print(f"  {str(bin_label):<10} {int(row['count']):>8} {pct:>13.1f}% {clr(float(row['pnl']), '+,.0f'):>23}{flag}")
    ...
    print(f"  FTMO equity marge     : {GREEN if ftmo_equity_marge > 0 else RED}EUR {ftmo_equity_marge:,.0f}{RESET}  "
    ...
    print(f"  Worst single day      : EUR {worst_day_loss:+,.0f}  "
    ...
    df.drop(columns=["in_dt", "uit_dt", "trade_date", "month", "week", "lot_bin"],
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="XAUUSD 6-maanden backtest met FTMO constraints (lot≤6, dag≤€8k)"
    )
    parser.add_argument("--capital", type=float, default=160_000.0,
                        help="Startkapitaal (default: 160000)")
    parser.add_argument("--start", type=str, default="2025-11-17",
                        help="Startdatum (default: 2025-11-17 = 6 maanden geleden)")
    parser.add_argument("--end", type=str, default="2026-05-17",
                        help="Einddatum (default: 2026-05-17 = vandaag)")
    parser.add_argument("--v19", action="store_true",
                        help="Gebruik V19 config i.p.v. V20 (standaard: V20)")
    parser.add_argument("--v18", action="store_true",
                        help="Gebruik V18 config i.p.v. V20 (standaard: V20)")
