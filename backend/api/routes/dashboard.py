    ...
    return APIResponse(
        data={
            ...
            "risk": {
                ...
                "daily_loss_pct": round(daily_loss_used / FTMO_DAILY_LOSS_LIMIT * 100, 1)
                if FTMO_DAILY_LOSS_LIMIT
                else 0,
                "total_loss_limit": FTMO_TOTAL_LOSS_LIMIT,
                "total_loss_used": round(total_loss_used, 2),
                "total_loss_pct": round(total_loss_used / FTMO_TOTAL_LOSS_LIMIT * 100, 1)
                if FTMO_TOTAL_LOSS_LIMIT
                else 0,
...
async def monthly_pnl(_user: dict = Depends(get_current_user)):
    try:
        ...
        df["month"] = pd.to_datetime(df["opened_at"], utc=True).dt.tz_convert(None).dt.to_period("M").astype(str)
        by_month = (
            df.groupby("month")
            .agg(
                pnl=("pnl", "sum"),
            ...
            heatmap_data.append(
                {
                    ...
                    "profit_factor": round(float(pnl[pnl > 0].sum()) / float(pnl[pnl < 0].abs().sum()), 2)
                    if not pnl[pnl < 0].empty and pnl[pnl < 0].abs().sum() > 0
                    else 99.0,
                }
            )
